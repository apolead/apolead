...
const Scripting = () => {
  ...
  const filteredScripts = scripts.filter(script => script.title.toLowerCase().includes(searchTerm.toLowerCase()) || script.type.toLowerCase().includes(searchTerm.toLowerCase()) || script.company.toLowerCase().includes(searchTerm.toLowerCase()) || script.description.toLowerCase().includes(searchTerm.toLowerCase()) || script.category.toLowerCase().includes(searchTerm.toLowerCase()));
  useEffect(() => {
  ...
  const renderStars = (rating: number) => {
    return Array(5).fill(0).map((_, i) => <Star key={i} className={`h-4 w-4 mr-1 text-yellow-400 fill-yellow-400`} />);
  };
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredScripts.map(script => <div key={script.id} className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden hover:shadow-lg transition duration-300 transform hover:-translate-y-1 cursor-pointer" onClick={() => openModal(script.id)}>
                <div className="p-5">
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl mx-auto my-8 overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto max-h-[70vh]">
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6 border border-gray-200">
                {script.content.sections.map((section, idx) => <div key={`section-${script.id}-${idx}`} className="mb-4">
              ...
              <div className={`${script.title === "Hard Sell" ? "bg-yellow-50 border-yellow-200" : "bg-blue-50 border-blue-200"} p-4 rounded-lg mb-6 border`}>
                ...
                <div className="text-gray-700 text-sm space-y-4">
                  {script.content.objections.map((obj, idx) => <div key={`objection-${script.id}-${idx}`}>
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6 border border-gray-200">
                {script.content.transfer.map((transfer, idx) => <div key={`transfer-${script.id}-${idx}`} className="mb-4">
