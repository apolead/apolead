...
const AdditionalTraining: React.FC = () => {
  ...
  const openVideoModal = (video: { id: string; title: string }) => {
        ...
        <div className="welcome-container max-w-4xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredVideos.map((video, index) => (
                <div 
                  ...
                  onClick={() => openVideoModal({ id: video.id, title: video.title })}
                >
                  ...
                  <div className="p-4">
                    <div className="flex flex-wrap gap-2 mb-2">
                      {video.tags.map((tag, tagIndex) => (
                        <span 
                          key={tagIndex}
                          className={`${tag.bgColor} ${tag.textColor} text-xs px-2 py-1 rounded-full`}
                        >
                          {tag.name}
                        </span>
                      ))}
          ...
          <div className="mt-8 text-center">
            <button className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-6 py-3 rounded-lg shadow transition">
              <BookOpen className="inline-block mr-2" size={16} /> Explore Scripts
            </button>
          </div>
        </div>
      </main>
