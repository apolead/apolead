...
const Dashboard = () => {
  ...
  const [isPolicyAcknowledgmentOpen, setIsPolicyAcknowledgmentOpen] = useState(false);
  ...
  const handleGetStarted = () => {
    ...
    // Check if policies have already been acknowledged
    if (userProfile?.telemarketing_policy_acknowledged && 
        userProfile?.do_not_call_policy_acknowledged && 
        userProfile?.policy_acknowledgment_name) {
      setShowBankingDialog(true);
    } else {
      setIsPolicyAcknowledgmentOpen(true);
    }
  };

  const handlePolicyAcknowledgmentComplete = async (acknowledgmentName: string) => {
    if (!user) return;
    ...
    try {
      await updateProfile({
        ...
        policy_acknowledgment_name: acknowledgmentName,
      ...
      setIsPolicyAcknowledgmentOpen(false);
      setShowBankingDialog(true);
    } catch (error) {
      console.error("Error updating profile with policy acknowledgments:", error);
      toast({
        title: "Error",
        description: "Failed to save your policy acknowledgments. Please try again.",
        variant: "destructive"
      });
    }
  };
