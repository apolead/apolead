  ...
  const handleContinue = async e => {
    ...
    try {
      ...
      const {
        data: profileData,
        error: profileError
      } = await supabase.from('user_profiles').select('gov_id_number').eq('gov_id_number', userData.govIdNumber).maybeSingle();
      ...
      } = await supabase.from('user_applications').select('gov_id_number').eq('gov_id_number', userData.govIdNumber).maybeSingle();
  ...
  const handleFileChange = async e => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.storage.from('government_ids').upload(filePath, file);
      if (error) {
        throw error;
      }
      ...
      const {
        data: urlData
      } = supabase.storage.from('government_ids').getPublicUrl(filePath);
  ...
  return <div className="flex flex-col md:flex-row w-full h-screen">
      ...
      <div className="w-full md:w-1/2 bg-[#1A1F2C] text-white relative p-8 md:p-16 flex flex-col justify-between overflow-hidden">
        ...
        <div className="relative z-10">
          <a href="#" onClick={handleBackToHome} className="inline-flex items-center text-white hover:text-white/80 mb-12">
      ...
      <div className="w-full md:w-1/2 bg-white p-8 md:p-16 flex flex-col">
        ...
        {errorMessage && <div className="bg-red-50 border border-red-200 text-red-600 p-3 rounded-md text-sm mb-6">
            {errorMessage}
          </div>}
        ...
        <form onSubmit={handleContinue} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input type="text" id="step1-firstName" value={userData.firstName} onChange={e => updateUserData({
              firstName: e.target.value
            })} className="w-full" placeholder="Enter first name" />
            </div>
            
            <div>
              <label htmlFor="step1-lastName" className="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
              <Input type="text" id="step1-lastName" value={userData.lastName} onChange={e => updateUserData({
              lastName: e.target.value
            })} className="w-full" placeholder="Enter last name" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input type="email" id="step1-email" value={userData.email} readOnly className="w-full bg-gray-100 cursor-not-allowed" />
            </div>
            
            <div>
              <label htmlFor="step1-birthDay" className="block text-sm font-medium text-gray-700 mb-1">Birth Day</label>
              <Input type="date" id="step1-birthDay" value={userData.birthDay} onChange={e => updateUserData({
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-md p-4 text-center bg-gray-50">
              <input type="file" id="govIdImage" accept="image/*,.pdf" className="hidden" onChange={handleFileChange} disabled={isUploading} />
              <label htmlFor="govIdImage" className="cursor-pointer">
                {isUploading ? <div className="flex flex-col items-center">
                    ...
                    <p className="text-sm text-gray-500 mb-1">
                      {userData.govIdImageUrl ? `File uploaded successfully` : 'Drag and drop your ID here, or click to browse'}
                    </p>
          ...
          <div>
            <label htmlFor="step1-govIdNumber" className="block text-sm font-medium text-gray-700 mb-1">
              Government ID Number
              {isCheckingGovId && <span className="ml-2 inline-flex items-center text-amber-500">
                  <Loader2 className="h-3 w-3 animate-spin mr-1" />
                  Validating...
                </span>}
            </label>
            <Input type="text" id="step1-govIdNumber" value={userData.govIdNumber} onChange={e => updateUserData({
            govIdNumber: e.target.value
          })} className={`w-full ${isCheckingGovId ? 'bg-gray-50' : ''}`} placeholder="Enter your ID number" disabled={isCheckingGovId} />
          ...
          <div className="flex justify-between mt-8">
            ...
            <Button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white" disabled={isCheckingGovId || isUploading}>
